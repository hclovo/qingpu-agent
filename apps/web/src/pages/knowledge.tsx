  ...
  return (
    <>
      <PageHeader
        ...
        actions={<button type="button" className="button primary" onClick={() => setOpen(true)}><Plus size={17} /> 新增知识</button>}
      />
      <section className="knowledge-stats">
        <div><BookOpenCheck size={20} /><span>知识条目<strong>{items.length}</strong></span></div>
        <div><Check size={20} /><span>当前可用<strong>{stats.ready}</strong></span></div>
        <div><FileText size={20} /><span>需人工复核<strong>{stats.review}</strong></span></div>
        <div><Link2 size={20} /><span>来源数量<strong>{stats.source}</strong></span></div>
      </section>
      <section className="card knowledge-card">
        <div className="toolbar">
          <form className="search-control wide" onSubmit={submitSearch}><Search size={16} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="检索标题、内容、标签或来源" /><button type="submit">检索</button></form>
        ...
        {loading ? <LoadingState /> : error ? <ErrorState message={error} onRetry={() => load(search)} /> : items.length ? (
          <div className="knowledge-grid">
            {items.map((item) => <article className="knowledge-item" key={item.id}>
              <div className={`knowledge-icon type-${item.type}`}>{item.type === 'url' ? <Link2 /> : item.type === 'file' ? <FileJson /> : <FileText />}</div>
              <div className="knowledge-main"><div className="knowledge-title-line"><h2>{item.title}</h2><StatusBadge value={item.status} label={knowledgeStatusLabels[item.status] ?? item.status} /></div><p>{item.content || item.source || '暂无内容摘要'}</p><div className="knowledge-meta"><span>{displaySource(item.sourceType ?? item.sourceKind)}</span><span>更新于 {formatDate(item.updatedAt)}</span>{(item.source ?? item.sourceUrl ?? item.sourcePath) && <span title={item.source ?? item.sourceUrl ?? item.sourcePath}>{item.source ?? item.sourceUrl ?? item.sourcePath}</span>}</div>{!!item.tags?.length && <div className="tag-row"><Tags size={13} />{item.tags.map((tag) => <span key={tag}>{tag}</span>)}</div>}</div>
            </article>)}
          </div>
        ) : <EmptyState title={search ? '没有匹配的知识' : '知识库还是空的'} description={search ? '尝试更换关键词，或补充新的知识条目。' : '添加文本、网页或文本类文件，为 Agent 建立企业上下文。'} action={<button className="button primary small" type="button" onClick={() => setOpen(true)}><Plus size={15} /> 新增知识</button>} />}
      </section>
...
function KnowledgeForm({ onCreated }: { onCreated: (item: KnowledgeItem) => void }) {
  ...
  return <form className="knowledge-form" onSubmit={submit}>
    <div className="type-picker">
      <button type="button" className={type === 'text' ? 'active' : ''} onClick={() => setType('text')}><FileText size={18} /><span>文本</span></button>
      <button type="button" className={type === 'url' ? 'active' : ''} onClick={() => setType('url')}><Link2 size={18} /><span>网页 URL</span></button>
      <button type="button" className={type === 'file' ? 'active' : ''} onClick={() => setType('file')}><UploadCloud size={18} /><span>本地文件</span></button>
    </div>
    ...
    {type === 'file' && <label className="file-drop"><input type="file" accept=".txt,.md,.csv,.json,text/plain,text/markdown,text/csv,application/json" onChange={(event) => void selectFile(event.target.files?.[0])} /><UploadCloud size={26} /><strong>{fileName || '选择或拖入文本类文件'}</strong><span>.txt / .md / .csv / .json，最大 2MB</span>{fileContent && <em>已读取 {fileContent.length.toLocaleString()} 个字符</em>}</label>}
    ...
    <div className="data-note"><Check size={15} /> 请勿上传密钥、未授权个人信息或未经核验的技术承诺。</div>
