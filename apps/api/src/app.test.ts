...
describe('Hono API', () => {
  ...
  it('健康检查明确展示规则模式并返回 requestId', async () => {
    ...
    expect(await response.json()).toMatchObject({ status: 'ok', agentMode: 'rules', version: '0.1.0', requestId: expect.any(String) })
  })
  ...
  it('无密钥时对话与自动发现稳定降级且显著标识演示数据', async () => {
    const app = createApp()
    const chat = await app.request('/api/agent/chat', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ message: '今天应该联系哪些客户和上游供应商？' }) })
    expect(await chat.json()).toMatchObject({ mode: 'rules' })
    const discovery = await app.request('/api/opportunities/discover', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ query: '船舶 氢能', days: 90 }) })
  ...
  it('支持补充知识并立即检索', async () => {
    const app = createApp()
    const create = await app.request('/api/knowledge', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ title: '低温工况补充', type: 'text', content: '北方重卡项目要求零下30度启动能力待售前确认', tags: ['重卡', '低温'] }) })
  ...
  it('拒绝非法请求并支持阶段更新', async () => {
    const app = createApp()
    const invalid = await app.request('/api/opportunities/analyze', { method: 'POST', headers: { 'content-type': 'application/json' }, body: '{}' })
    expect(invalid.status).toBe(400)
    const updated = await app.request('/api/opportunities/opp-river-vessel/stage', { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ stage: 'engaging' }) })
  ...
  it('筛选参数和无效 JSON 均返回带 requestId 的 400，而不是静默空列表或 500', async () => {
    ...
    const malformed = await app.request('/api/agent/chat', { method: 'POST', headers: { 'content-type': 'application/json' }, body: '{' })
  ...
  it('知识录入保留文本来源，并拒绝缺失来源的 URL/文件记录', async () => {
    ...
    for (const input of [
      ...
      const response = await app.request('/api/knowledge', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(input) })
  ...
  it('总览与简报严格按 A 级统计高潜，并保留来源兼容字段', async () => {
    ...
    const briefing = await (await app.request('/api/agent/briefing')).json() as { highPotentialOpportunities: Array<{ grade: string }> }
  ...
  it('重复研判返回 409，缺失详情返回 404', async () => {
    ...
    expect((await app.request('/api/opportunities/analyze', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(input) })).status).toBe(201)
    const duplicate = await app.request('/api/opportunities/analyze', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(input) })
  ...
  it('智能调用异常时安全降级并继续强调人工确认边界', async () => {
    ...
    const response = await app.request('/api/agent/chat', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ message: '今天应该联系谁？' }) })
  ...
  it('核心资源响应可通过共享 contracts 的 Zod Schema 校验', async () => {
    ...
    AgentBriefingSchema.parse(await getJson('/api/agent/briefing'))
    const chat = await app.request('/api/agent/chat', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ message: '请介绍船用产品' }) })
